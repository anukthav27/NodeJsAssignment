...
jest.mock('mongodb', () => {
  ...
  return {
    MongoClient: { connect: jest.fn(() => Promise.resolve(mClient)) },
    ObjectId: jest.fn(),
  };
});
...
describe('POST /users', () => {
  ...
  it('should create a new user successfully', async () => {
    ...
    db.collection('users').insertOne.mockResolvedValue({
  ...
  it('should return 400 if name or email is missing', async () => {
    const user = { name: 'Alice Johnson' }; // Missing email

    const response = await request(app).post('/users').send(user).expect(400);
  ...
  it('should return 500 if there is a server error', async () => {
    ...
    db.collection('users').insertOne.mockRejectedValue(
      new Error('Database error')
    );

    const response = await request(app).post('/users').send(user).expect(500);
...
describe('PUT /users/:id', () => {
  ...
  it('should return 404 if the user is not found', async () => {
    const userId = 'mockedUserId';
    const updates = { name: 'Alice Johnson Updated' };
